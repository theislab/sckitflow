...
## ✅ Validation loop + held-out split shipped **[P0.3/P1.1/P1.2/P1.3]**

`FlowMatching.fit(split_by=…, split_ratios=…, metrics=…, valid_freq=…)` now trains with a held-out
validation split and scores distribution metrics on it. What landed:

- **Held-out split** — `split_by` (condition column(s)) + `split_ratios` (`(train, val)` sequence or a
  `{"train":…, "val":…}` mapping, default `(0.8, 0.2)`) partition **whole conditions** via binded's
  `split_scheme` (seed-deterministic — the val leaves are disjoint from train, no cell leakage). The train
  `Loader` is built from the `train` split; `split_by=None` keeps the old all-conditions, no-validation path
  (byte-identical — all 14 prior tests still pass).
- **`RSquared` metric** (`metrics/_metrics.py`, registry `"r_squared"`) — per-condition R² between the
  **predicted and target feature-wise means** (cellflow's `r_squared`), averaged over conditions. Sits next
  to `EnergyDistance` (`"e-dist"`). *Note:* R² needs feature-varying gene means to be well-behaved (real
  scRNA is; a constant-shift toy makes the denominator degenerate — not a bug).
- **`validation_step` in the shared harness** — every `valid_freq` steps (step-based `val_check_interval`,
  no sanity pass) binded's `EvalLoader` yields one held-out **condition** per batch (`source`/`target`/
  `condition`/`leaf`); the harness translates the controls and updates the torchmetrics, and
  `on_validation_epoch_end` logs `val_<metric>_mean` (the sweep objective) and appends to
  `model.metrics_history` (`{metric: [mean per pass]}`). Backward-compatible: no split ⇒ no val loop.
- **Shared inference** — the ODE integration was factored out of `FlowMatching.predict` into
  `training/_predict.py::integrate_translation` (OTFM integrates the cells; GENOT integrates from latent
  noise conditioned on the source cell), so the **validation metric reflects exactly what `predict` does**.
  predict was rewritten onto it, bit-identically (save/load + genot repro tests unchanged).
- **Tests:** `test_r_squared_metric` (hand-computed R²), `test_fit_validation_loop[otfm|genot]` (history
  keys/count/finite + train+validate bit-reproducible), `test_fit_split_holds_out_whole_conditions`
  (no leakage), `test_split_ratios_forms_and_validation`, `test_fit_unknown_metric_raises`.

## Next up (todo)

Ordered for a faithful cf-train Tahoe run (the ⏭ rows of `cellflow-vendor-fm.md` §8), then model polish:

1. **Logger passthrough** — a `logger=` param on `fit` (None / wandb) so `val_<metric>_mean` reaches the
   sweep tracker (currently `logger=False`; metrics are on `model.metrics_history`). **[P2.1]**
2. **Classifier-free guidance** (GENOT + OTFM) — condition-dropout in training + a guided `predict`. **[G3]**
