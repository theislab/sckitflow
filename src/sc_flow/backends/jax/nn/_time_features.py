...
from sc_flow.backends.jax._types import ArrayLike, TTimeFeaturesFn
from sc_flow.backends.jax._utils import ensure_2d_tensor_with_singleton_trailing_dim
...
def ott_jax_time_features(
    ...
    :param t: The current continuous time index $t$ on which the features $\tilde{t}_K$ are computed.
        Must be an array of shape $()$, $(B, )$ or $(B, 1)$, where $B$ denotes the batch size.
    :type t: sc_flow.backends.jax._types.ArrayLike
...
def torch_cfm_time_features(
    ...
    r"""Applies the sinusoidal time features on the current time index as defined in the TorchCFM paper.
...
def make_custom_time_features(
    ...
    :type time_features_fn: sc_flow.backends.jax._types.TTimeFeaturesFn | None
    ...
    def _time_features_fn(t: ArrayLike):
        t = ensure_2d_tensor_with_singleton_trailing_dim(t)
...
def get_time_features_fn(
    ...
    It supports initialization of time features functions in the following two ways:
        ...
        2) It is otherwise possible to initialize custom time featurizers by passing
            the :param: `time_features_fn`, which needs to be a :class: `Callable[ArrayLike, int, ...]`
