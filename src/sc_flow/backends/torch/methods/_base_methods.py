...
class BaseGenerativeFlow(BaseMethod):
    ...
    def _extract_coupling_data(
        ...
        state_lin: torch.Tensor = coupling_data.get("state_lin")
        state_quad: torch.Tensor = coupling_data.get("state_quad")
    ...
    def _extract_step_data(
        ...
        target_coupling_lin, target_coupling_quad = self._extract_coupling_data(target_data_dict, "target")
        target_state_data = self._extract_state_data(target_data_dict)
        target_condition_data = target_data_dict.get("condition_data")
        target_group_data = target_data_dict.get("group_data")

        # optionally parse target data dictionary
        if source_data_dict is not None:
            source_coupling_lin, source_coupling_quad = self._extract_coupling_data(source_data_dict, "source")
            source_state_data = self._extract_state_data(source_data_dict)
        else:
    ...
    def _extract_matched_observations(
        ...
        if step_data.source_state is not None:
