...
class EnergyDistance(Metric):
    ...
    def __init__(self, dist_sync_on_step=False):
        ...
        self.add_state("energy_distance_raw", default=torch.tensor(0., dtype=torch.float32), dist_reduce_fx="sum")
    ...
    def update(self, pred: torch.Tensor, target: torch.Tensor):
        ...
        self.energy_distance_raw += (2. * delta - sigma_pred - sigma_target)
        self.total += 1
...
class MaximumMeanDiscrepancy(Metric):
    ...
    def __init__(self, gammas: list[float] = None, dist_sync_on_step=False):
        ...
        self.add_state("mmd_raw", default=torch.tensor(0., dtype=torch.float32), dist_reduce_fx="sum")
        self.add_state("total", default=torch.tensor(0., dtype=torch.float32), dist_reduce_fx="sum")
