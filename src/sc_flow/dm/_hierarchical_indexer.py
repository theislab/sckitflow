...
class HierarchicalIndexer:
    """Class for hierachical indexing of dataframes in subpopulations
    ...
    def _init_registry(
        ...
        self._registry: dict[str, tuple[str] | None] = {
            "groups": self.groups_cols if self.groups_cols is None else tuple(self.groups_cols),
            "conditions": self.conditions_cols if self.conditions_cols is None else tuple(self.conditions_cols),
        }
    ...
    def create_index(self, df: pd.DataFrame) -> pd.MultiIndex:
        ...
        for level_name, level_cols in self._registry.items():
            ...
            # use level data when provided
                ...
                ordered_cols = sorted(level_cols) if self.sort_columns else level_cols
                ...
                if missing:
                    ...
                    raise KeyError(msg)
                level_data = df[ordered_cols].to_numpy()
