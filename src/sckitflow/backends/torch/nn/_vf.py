...
from sckitflow._constants import (
...
from sckitflow.backends.torch._types import MappedTensor, TConditioningFn, TTimeFeaturesFn, TVfFn
from sckitflow.backends.torch._utils import make_concatenation_possible
from sckitflow.backends.torch.nn._conditioning_layers import BaseConditioningLayer, get_conditioning_layer
from sckitflow.backends.torch.nn._modules import BaseModule, FunctionalModule
from sckitflow.backends.torch.nn._set_encoder import SetEncoder
from sckitflow.backends.torch.nn._time_features import get_time_features_fn
from sckitflow.backends.torch.nn._utils import init_module_from_dict
...
class MLPVelocity(BaseVelocityField):
    ...
    def __init__(
        ...
        :param num_time_features: (Optional) Sets the number of resulting time features, hence it must be even.
            ...
            :constant: `sckitflow._constants.DEFAULT_NUM_TIME_FEATURES`. Defaults to `None`.
        ...
        :param max_period: (Optional) Sets the value of $M$, used for the linear scaling of the time features.
            ...
            When not provided, it will be set to :constant: `sckitflow._constants.DEFAULT_TIME_FEATURES_MAX_PERIOD`. Defaults to `None`.
        ...
        :param state_encoder_output_dim: (Optional) The output dimensionality for the state encoder, only used when :param: `encode_state`
            is set to `True`. When `None` it will be set to :constant: `sckitflow._constants.DEFAULT_VF_LATENT_STATE_DIM`.
            Defaults to `None`.
        ...
        :param time_encoder_output_dim: (Optional) The output dimensionality for the time encoder, only used when :param: `encode_time`
            is set to `True`. When `None` it will be set to :constant: `sckitflow._constants.DEFAULT_VF_LATENT_TIME_DIM`.
            Defaults to `None`.
        ...
        :param conditioning_id: (Optional) String identifier indicating the type of conditioning applied to the states.
            ...
            :constant: `sckitflow._constants.DEFAULT_CONDITIONING_LAYER`.
    ...
    def _get_num_time_features(
        ...
        :constant: `sckitflow._constants.DEFAULT_NUM_TIME_FEATURES`.
        """
