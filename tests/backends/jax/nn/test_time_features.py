...
def valid_time_features_fn(
    ...
    return jnp.ones((t.shape[0], num_time_features))
...
def invalid_time_features_fn(
    ...
    return jnp.ones((t.shape[0], num_time_features + 1))


class TestTimeFeatures:
    @pytest.mark.parametrize("t", [jnp.zeros(()), jnp.zeros((batch_size, 1)), jnp.zeros((batch_size,))])
    ...
    def test_get_time_features_fn(
        ...
        if (time_features_id is not None) or (time_features_fn is not None):
            ...
            assert jnp.array_equal(t_features.squeeze(), t.squeeze())
