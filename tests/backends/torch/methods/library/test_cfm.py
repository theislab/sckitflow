...
def cfm_instance():
    ...
    cfm = CFM(dims_registry=dims_reg, dm=dm, dtype=torch.float32, device_id="cpu")
...
class TestCFM:
    ...
    def test_train_step(self, cfm_instance):
        matched_distr = Mock()
        # Create a proper step_data with tensors
        step_data = StepData(
            target_state=torch.randn(4, 2),
        ...
        # Replace the matching function with a simple identity mapper
        cfm_instance._match_fn = lambda source_lin, target_lin, source_quad, target_quad: (np.arange(4), np.arange(4))
