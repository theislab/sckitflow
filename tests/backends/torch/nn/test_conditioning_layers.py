...
from sckitflow.backends.torch.nn._conditioning_layers import (
    ConcatConditioning,
...
class TestConditioningLayers:
    ...
    def test_concat_conditioning(self, concat_condition: bool) -> None:
        ...
        # case 1
        t = torch.zeros((1, num_time_feats))
        cond = torch.zeros((1, num_cond_feats))
        x = torch.zeros((batch_size, num_feats))
        if concat_condition:
            out = conditioning(t, x, cond)
            assert out.shape == ((batch_size, num_feats + num_time_feats + num_cond_feats))
        ...
        cond = torch.zeros((batch_size, num_cond_feats))
        x = torch.zeros((batch_size, num_feats))
        if concat_condition:
            out = conditioning(t, x, cond)
            assert out.shape == ((batch_size, num_feats + num_time_feats + num_cond_feats))
        ...
        cond = torch.zeros((batch_size, num_samples, num_cond_feats))
        x = torch.zeros((batch_size, num_samples, num_feats))
        if concat_condition:
            out = conditioning(t, x, cond)
            assert out.shape == ((batch_size, num_samples, num_feats + num_time_feats + num_cond_feats))
