...
class TestNNUtils:
    ...
    def test_init_module_from_dict(
        ...
        if input_dim is None and "input_dim" not in layers_dict.keys():
            with pytest.raises(KeyError, match=r"Layers dictionary must contain \"input_dim\" when it is not passed explicitly\."):
        ...
        if output_dim is None and "output_dim" not in layers_dict.keys():
            with pytest.raises(KeyError, match=r"Layers dictionary must contain \"output_dim\" when it is not passed explicitly\."):
                layer = init_module_from_dict(layers_dict, input_dim=input_dim, output_dim=output_dim)
            return None
