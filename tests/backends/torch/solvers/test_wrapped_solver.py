...
def to_tx(t: Tensor) -> TorchaxTensor:
    """Convert plain torch tensor to torchax tensor."""
    return t.to("jax")


def to_torch(t: TorchaxTensor) -> Tensor:
    """Convert torchax tensor to plain CPU torch tensor."""
    import numpy as np
...
@pytest.fixture
def y0() -> TorchaxTensor:
    return to_tx(torch.tensor([[0.0, 1.0], [2.0, -3.0]]))
...
def test_solve_returns_trajectory_shape_dtype(poly_vf: PolyTimeVF, y0: TorchaxTensor, t01: Tensor) -> None:
    ...
    assert isinstance(traj, TorchaxTensor)
    assert traj.shape == (t01.numel(), *y0.shape)
    assert traj.dtype == y0.dtype


def test_vf_kwargs_forwarded(poly_vf: PolyTimeVF, y0: TorchaxTensor, t01: Tensor) -> None:
    vf_kwargs = {"condition_dict": {"foo": to_tx(torch.ones(1, 2))}, "alpha": 0.42}
...
def test_many_integrators_match_analytic_solution(method: str) -> None:
    y_init = to_tx(torch.tensor([1.0, 0.0, -2.0]))
    y_init_cpu = torch.tensor([1.0, 0.0, -2.0])

    solver = ODESolver(dynamics=PolyTimeVF(), method=method, device_id="cpu")
    ...
    assert torch.allclose(y1, y_init_cpu + 1.0, atol=atol, rtol=0.0)


def test_time_tensor_respected_uniform_grid(poly_vf: PolyTimeVF) -> None:
    y_init = to_tx(torch.tensor([0.5, -1.5]))
    y_init_cpu = torch.tensor([0.5, -1.5])
    ...
    assert torch.allclose(to_torch(traj[-1]), y_init_cpu + 1.0, atol=1e-2, rtol=0.0)


def test_solver_kwargs_forwarded_to_diffrax_dt0(poly_vf: PolyTimeVF) -> None:
    y_init = to_tx(torch.zeros(1))
    y_init_cpu = torch.zeros(1)
    ...
    err_default = (to_torch(traj_default[-1]) - (y_init_cpu + 1.0)).abs().item()
    err_small = (to_torch(traj_small_dt[-1]) - (y_init_cpu + 1.0)).abs().item()

    assert err_small < err_default
    assert not torch.allclose(to_torch(traj_default[-1]), to_torch(traj_small_dt[-1]), atol=1e-6, rtol=0.0)


def test_runs_on_cpu(poly_vf: PolyTimeVF, y0: TorchaxTensor, t01: Tensor) -> None:
    solver = ODESolver(dynamics=poly_vf, method="euler", device_id="cpu")
    out = solver.solve(
        source=y0,
        t0=float(t01[0].item()),
...
def test_runs_on_cuda_when_available(poly_vf: PolyTimeVF, y0: TorchaxTensor, t01: Tensor) -> None:
...
def test_stepsize_controller_constant_pass_through(poly_vf: PolyTimeVF) -> None:
    y0 = to_tx(torch.tensor([0.25, -1.25, 2.0]))
    y0_cpu = torch.tensor([0.25, -1.25, 2.0])
    ...
    out = solver.solve(
        ...
        num_time_steps=201,
        return_trajectory=True,
        solver_kwargs={"stepsize_controller": ConstantStepSizeController(), "dt0": 0.01},
    )

    assert isinstance(out, TorchaxTensor)
    assert out.shape == (201, *y0.shape)
    assert torch.allclose(to_torch(out[-1]), y0_cpu + 1.0, atol=2e-2, rtol=0.0)


def test_stepsize_controller_pid_pass_through(poly_vf: PolyTimeVF) -> None:
    y0 = to_tx(torch.tensor([1.0, 0.0, -2.0]))
    y0_cpu = torch.tensor([1.0, 0.0, -2.0])
    ...
    out = solver.solve(
        ...
        num_time_steps=301,
        return_trajectory=True,
        solver_kwargs={"stepsize_controller": PIDController(rtol=1e-6, atol=1e-8), "dt0": 0.05},
    )

    assert isinstance(out, TorchaxTensor)
    assert out.shape == (301, *y0.shape)
    assert torch.allclose(to_torch(out[-1]), y0_cpu + 1.0, atol=1e-2, rtol=0.0)


def test_stepsize_controller_step_to_pass_through(poly_vf: PolyTimeVF) -> None:
    y0 = to_tx(torch.tensor([0.5, -1.5]))
    y0_cpu = torch.tensor([0.5, -1.5])
    ...
    out = solver.solve(
        ...
        num_time_steps=101,
        return_trajectory=True,
        solver_kwargs={"stepsize_controller": StepTo(ts=ts)},
    )
...
def test_stepsize_controller_clip_pass_through(poly_vf: PolyTimeVF) -> None:
    y0 = to_tx(torch.tensor([0.0]))
    y0_cpu = torch.tensor([0.0])
...
def test_gradients_through_solver_constant_vf(constant_vf: ConstantVF) -> None:
    ...
    final = solver.solve(source=y_init, t0=0.0, t1=1.0, num_time_steps=201, solver_kwargs={}, return_trajectory=False)
    final.sum().backward()
...
def test_gradients_source(poly_vf: PolyTimeVF) -> None:
    y_init = to_tx(torch.tensor([1.0, 0.0, -2.0])).requires_grad_(True)
    solver = ODESolver(dynamics=poly_vf, method="euler", device_id="cpu")

    final = solver.solve(source=y_init, t0=0.0, t1=1.0, num_time_steps=100, solver_kwargs={}, return_trajectory=False)
    final.sum().backward()

    assert y_init.grad is not None, "grad did not reach source"
